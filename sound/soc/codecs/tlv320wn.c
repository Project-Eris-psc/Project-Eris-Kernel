...
#define ADC3_PGA_CH1_GAIN    58
#define ADC3_PGA_CH2_GAIN    0
#endif

#ifdef CONFIG_SND_SOC_4_ADCS
unsigned int dmic_cfg = 0;
#else
...
static int tlv320_regr_page0_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}


static int tlv320_regr_page0_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int tlv320_regr_page1_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int tlv320_regr_page1_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int reg = ucontrol->value.integer.value[0];
	unsigned short d2regvalue;
	int i = 0;
...
static int tlv320_adc_volume_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if(vol > 104) {
		vol = 104;
		printk("you set vol[%d] > max vaule(104),modify your vol == 104\n",vol);
	}
...
#if 0
static int tlv320_adc_pga_gain_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int tlv320_adc_pga_gain_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int gain = ucontrol->value.integer.value[0];

	int i = 0,Lchgain,Rchgain;

	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);

        for (i = 0; i < NUM_ADC3101; i++) {
		snd_soc_write(codec, ADC_LEFT_APGA_CTRL(i),gain);
		snd_soc_write(codec, ADC_RIGHT_APGA_CTRL(i),gain);

		Lchgain = snd_soc_read(codec,ADC_LEFT_APGA_CTRL(i))
		Rchgain = snd_soc_read(codec,ADC_LEFT_APGA_CTRL(i))

		printk("ADC[%d] Lchgain %d,Rchgain %d\n",i,Lchgain,Rchgain);
        }
	return 0;
}
#endif
...
#if 0
  	SOC_SINGLE_EXT("ADC PGA Gain Ctrl",
	0,
	0,
	80,
	0,
...
static void tlv320_PGA_Gain(struct snd_soc_codec *codec)
{
		...
		/*ADC3*/
		snd_soc_write(codec, ADC_LEFT_APGA_CTRL(3),ADC3_PGA_CH1_GAIN);
		snd_soc_write(codec, ADC_RIGHT_APGA_CTRL(3),ADC3_PGA_CH2_GAIN);
#endif
