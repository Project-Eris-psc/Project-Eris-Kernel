...
#define MTK_UART_WAIT_ACK_TIMES	1000
...
static int mtk8250_probe(struct platform_device *pdev)
{
	...
	/* get gpio20 status */
	err = gpio20 = of_get_named_gpio(dnode, "uart_gpio", 0);//get virtual gpio number
	if(err < 0)
		dev_err(&pdev->dev, "gpio20 is %d\n", gpio20);

	err = gpio_request(gpio20, "uart_gpio");
	if(!err)
		dev_err(&pdev->dev, "gpio20 request failed\n");

	err = gpio_direction_input(gpio20);
	mdelay(100);
	gpio20_status = gpio_get_value(gpio20);
	gpio_free(gpio20);
