	...
	for (i = start_blk; i <= MTK_GPT_END_BLK; i++) {
		...
		for (j = 0; j < (1 << (chip->phys_erase_shift - chip->page_shift)); j++) {
			...
			if (mtd->_read(mtd, addr, mtd->writesize, &retlen,  buf) < 0) {
				/* probably met a bad block, break page loop to check next block */
				break;
			}
			...
			if (header_found) {
				...
				while (gpt_e->partition_type_guid.b[0] != 0) {
					...
					#ifdef CONFIG_MTD_VERITY
					{
						...
						if (current_part == get_rootfs_mtd_num())
						{
							pr_err("[rootfs_check]set mtd num %d as read-only\n",current_part);
							mtk_part[current_part].mask_flags = MTD_WRITEABLE;
						}
					}
					#endif
					gpt_e++;
