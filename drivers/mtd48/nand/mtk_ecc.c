...
int mtk_ecc_enable_clk(struct mtk_ecc *ecc)
{
	return clk_prepare_enable(ecc->clk);
}
EXPORT_SYMBOL(mtk_ecc_enable_clk);

void mtk_ecc_disable_clk(struct mtk_ecc *ecc)
{
	clk_disable_unprepare(ecc->clk);
}
EXPORT_SYMBOL(mtk_ecc_disable_clk);
...
int mtk_ecc_wait_done(struct mtk_ecc *ecc, enum mtk_ecc_operation op)
{
	...
	if (op == ECC_DECODE) {
		...
		ret = readl_poll_timeout_atomic(ECC_REG(ecc, ECC_ENCIRQ_STA), enc,
					enc, 1, ECC_TIMEOUT);
	}
...
/* Compatible to MT8516 */
static const struct mtk_ecc_comp ecc_mt8516 = {
	.ecc_regs = mt8516_ecc_regs,
	.type = MTK_ECC_MT8516,
	.mode_shift = 5,
	.max_ecc_str = 19,
	.parity_bit = 14,
...
static const struct of_device_id mtk_ecc_dt_match[] = {
	{
		...
		.compatible = "mediatek,mt8167-ecc",
		.data = &ecc_mt8516,
	}, {
	},
};
