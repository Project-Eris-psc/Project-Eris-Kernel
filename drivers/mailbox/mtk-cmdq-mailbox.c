...
struct cmdq {
	...
	struct cmdq_priv        con_priv[CMDQ_THR_MAX_COUNT];
	struct clk		*clock;
	bool			suspended;
};
...
{
	struct cmdq_priv *con_priv = (struct cmdq_priv *)chan->con_priv;

	cmdq_task_exec(data, (struct cmdq_thread *)con_priv->priv);
	return 0;
}
		...
		const struct of_phandle_args *sp)
{
	...
	con_priv = (struct cmdq_priv *)mbox->chans[ind].con_priv;
	thread = (struct cmdq_thread *)con_priv->priv;
...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	plat = (u32)(unsigned long)of_device_get_match_data(dev);
	...
	for (i = 0; i < ARRAY_SIZE(cmdq->thread); i++) {
		...
		cmdq->con_priv[i].priv = (void *)&cmdq->thread[i];
		cmdq->con_priv[i].plat = plat;
		cmdq->mbox.chans[i].con_priv = (void *)&cmdq->con_priv[i];
	}
...
static const struct of_device_id cmdq_of_ids[] = {
	{.compatible = "mediatek,mt8173-gce", .data = (void *)8173},
	{.compatible = "mediatek,mt2712-gce", .data = (void *)2712},
	{.compatible = "mediatek,mt8167-gce", .data = (void *)8167},
	{}
};
