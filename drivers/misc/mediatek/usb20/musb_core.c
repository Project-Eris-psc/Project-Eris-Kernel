...
/* charger_info: 1 : SDP */
module_param_named(charger_info, musb_charger_info, uint, S_IRUGO);
...
#ifdef CONFIG_MTK_MUSB_PORT0_WAKELOCK_FORCE_SUSPEND

void musb_show_clk_prepared_info(int required_prepared)
{
#undef FORCE_LOG
	struct clk_hw *c_hw;
	int curr_clk_prepared;

	if (usbpll_clk) {
		c_hw = __clk_get_hw(usbpll_clk);
		curr_clk_prepared = clk_hw_is_prepared(c_hw);
		#ifdef FORCE_LOG
			DBG(0, "usbpll prepare warn: curr:%d, required:%d\n",
				curr_clk_prepared, required_prepared);
		#endif
		if (curr_clk_prepared != required_prepared)
			DBG(0, "usbpll prepare warn: curr:%d, required:%d\n",
				curr_clk_prepared, required_prepared);
	}

	if (usb_clk) {
		c_hw = __clk_get_hw(usb_clk);
		curr_clk_prepared = clk_hw_is_prepared(c_hw);
		#ifdef FORCE_LOG
			DBG(0, "usb_clk prepare warn: curr:%d, required:%d\n",
				curr_clk_prepared, required_prepared);
		#endif
		if (curr_clk_prepared != required_prepared)
			DBG(0, "usb prepare warn: curr:%d, required:%d\n",
				curr_clk_prepared, required_prepared);
	}

	if (usbmcu_clk) {
		c_hw = __clk_get_hw(usbmcu_clk);
		curr_clk_prepared = clk_hw_is_prepared(c_hw);
		#ifdef FORCE_LOG
			DBG(0, "usbmcu_clk prepare warn: curr:%d, required:%d\n",
				curr_clk_prepared, required_prepared);
		#endif
		if (curr_clk_prepared != required_prepared)
			DBG(0, "usbmcu prepare warn curr:%d, required:%d\n",
				curr_clk_prepared, required_prepared);
	}
#undef FORCE_LOG
}

void musb_show_clk_enable_count(unsigned int require_en_cnt, int compare_type)
{
#undef FORCE_LOG
	unsigned int curr_en_cnt;

	if (usbpll_clk) {
		curr_en_cnt = __clk_get_enable_count(usbpll_clk);
		#ifdef FORCE_LOG
			DBG(0, "usbpll en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		#endif
		if (compare_type == 1 && curr_en_cnt <= require_en_cnt)
			DBG(0, "usbpll en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		else if (compare_type == 0 && curr_en_cnt != require_en_cnt)
			DBG(0, "usbpll en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		else if (compare_type == -1 && curr_en_cnt >= require_en_cnt)
			DBG(0, "usbpll_clk en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
	}

	if (usb_clk) {
		curr_en_cnt = __clk_get_enable_count(usb_clk);
		#ifdef FORCE_LOG
			DBG(0, "usb_clk en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		#endif
		if (compare_type == 1 && curr_en_cnt <= require_en_cnt)
			DBG(0, "usb en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		else if (compare_type == 0 && curr_en_cnt != require_en_cnt)
			DBG(0, "usb en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
	...
	if (usbmcu_clk) {
		curr_en_cnt = __clk_get_enable_count(usbmcu_clk);
		#ifdef FORCE_LOG
			DBG(0, "usbmcu_clk en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		#endif
		if (compare_type == 1 && curr_en_cnt <= require_en_cnt)
			DBG(0, "usbmcu en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
		else if (compare_type == 0 && curr_en_cnt != require_en_cnt)
			DBG(0, "usbmcu en_cnt warn: curr:%u, required:%u\n",
				curr_en_cnt, require_en_cnt);
...
static void musb_clk_disable_all(void)
{
	struct clk_hw *c_hw;
	int i;
	...
	if(usbmcu_clk) {
		i = 0;
		c_hw = __clk_get_hw(usbmcu_clk);
		while(c_hw && __clk_get_enable_count(usbmcu_clk) > 0) {
			clk_disable(usbmcu_clk);
			++i;
		}
		DBG(1, "clk_disable usbmcu_clk count:%d, enable_count:%u\n", i, __clk_get_enable_count(usbmcu_clk));
	}

	if(usb_clk) {
		i = 0;
		c_hw = __clk_get_hw(usb_clk);
		while(c_hw && __clk_get_enable_count(usb_clk) > 0) {
			clk_disable(usb_clk);
			++i;
		}
		DBG(1, "clk_disable usb_clk count:%d, enable_count:%u\n", i, __clk_get_enable_count(usb_clk));
	}

	if(usbpll_clk) {
		i = 0;
		c_hw = __clk_get_hw(usbpll_clk);
		while(c_hw && __clk_get_enable_count(usbpll_clk) > 0) {
			clk_disable(usbpll_clk);
			++i;
		}
		DBG(1, "clk_disable usbpll_clk count:%d, enable_count:%u\n", i, __clk_get_enable_count(usbpll_clk));
	}
}

static void musb_clk_unprepare_all(void)
{
	struct clk_hw *c_hw;
	int i;
	...
	if(usbmcu_clk) {
		i = 0;
		c_hw = __clk_get_hw(usbmcu_clk);
		while(c_hw && clk_hw_is_prepared(c_hw)) {
			clk_unprepare(usbmcu_clk);
			++i;
		}
		DBG(1, "clk_unprepare usbmcu_clk count:%d, is_prepared:%d\n", i, clk_hw_is_prepared(c_hw));
	}
	...
	if(usb_clk) {
		...
		DBG(1, "clk_unprepare usb_clk count:%d, is_prepared:%d\n", i, clk_hw_is_prepared(c_hw));
	}
	...
	if(usbpll_clk) {
		...
		DBG(1, "clk_unprepare usbpll_clk count:%d, is_prepared:%d\n", i, clk_hw_is_prepared(c_hw));
	}
}
#endif
...
#ifdef CONFIG_MTK_MUSB_PORT0_WAKELOCK_FORCE_SUSPEND
	musb->power = false;
	musb_reset_usb_enable_count(0);

	musb_clk_disable_all();
	musb_clk_unprepare_all();

	musb_show_clk_prepared_info(0);
	musb_show_clk_enable_count(0, 0);
#endif
...
#ifdef CONFIG_MTK_MUSB_PORT0_WAKELOCK_FORCE_SUSPEND
	/* set the power flag for next usb enumeration */
	mtk_usb_power = false;
...
#ifdef CONFIG_MTK_MUSB_PORT0_RESUME_CONNECT_USB
	/* enter device mode: eg: adb */
	mt_usb_connect();
#endif
