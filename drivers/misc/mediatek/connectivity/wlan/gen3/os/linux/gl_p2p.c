	      ...
	      IN struct iw_request_info *prIwrInfo, OUT struct iw_freq *prIwFreq, IN char *pcExtra)
{
	...
	u4ChannelInKHz = nicChannelNum2Freq(prP2pConnReqInfo->rChannelInfo.ucChannelNum);

	if (u4ChannelInKHz == 0) {
		prIwFreq->m = 2412;		/* freq in MHz */
		prIwFreq->e = 0;
	} else {
		prIwFreq->m = u4ChannelInKHz;	/* freq in KHz */
		prIwFreq->e = 3;
	}
...
int p2pDoIOCTL(struct net_device *prDev, struct ifreq *prIfReq, int i4Cmd)
{
	...
	} else if (i4Cmd == SIOCSIWFREQ){ /* 0x8B04, set channel */
		DBGLOG(INIT, TRACE, "set channel: freq 0x%x\n", iwr->u.freq);
		ret = p2p_wext_set_freq(prDev, NULL, &iwr->u.freq, NULL);
	} else if (i4Cmd == SIOCGIWFREQ){ /* 0x8B05, get channel */
		/* Even we report a freq to upper layer, HotspotManager still set channel 6 */
		DBGLOG(INIT, TRACE, "get channel: freq m=%d, e=%d\n", iwr->u.freq.m, iwr->u.freq.e);
		ret = p2p_wext_get_freq(prDev, NULL, &iwr->u.freq, NULL);
	} else {
...
int p2pSetMACAddress(IN struct net_device *prDev, void *addr)
{
	...
	prAdapter = prGlueInfo->prAdapter;
	ASSERT(prAdapter);
	DBGLOG(P2P, WARN, "%s set p2p mac " MACSTR "\n", __func__, MAC2STR(sock_addr->sa_data));
