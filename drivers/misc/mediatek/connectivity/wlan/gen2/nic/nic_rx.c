...
VOID nicRxProcessMonitorPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	UINT_8 broadcast_mac[MAC_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
...
#ifdef CONFIG_ELIAN_SUPPORT
	sccb_rx(prSwRfb->pvHeader, prSwRfb->u2PacketLen-9);
#endif
	...
	/* process dmac is broadcast */
	if ((mac_header->u2FrameCtrl & MASK_TO_DS_FROM_DS) == MASK_FC_FROM_DS)
	{
		if (!EQUAL_MAC_ADDR(mac_header->aucAddr1, broadcast_mac))
			valid_pkt_flag = 0;
	}
	if ((mac_header->u2FrameCtrl & MASK_TO_DS_FROM_DS) == MASK_FC_TO_DS)
	{
		if (!EQUAL_MAC_ADDR(mac_header->aucAddr3, broadcast_mac))
			valid_pkt_flag = 0;
	}
	...
	skb_trim(prSkb, 0);
	skb_put(prSkb, prSwRfb->u2PacketLen + u4RadiotapLen);
