...
#define CMD_AUXADC_READ_ONE_CHAN  _IOWR('k', 5, int)
#define CMD_AUXADC_READ_ALL_EXT_CHAN _IOWR('k', 6, int)

/* AUXADC data format*/
#define AUXADC_CHANNEL_BITS     12
#define AUXADC_VALUE_BITS  0

#define AUXADC_PACK(nr,value) \
    (((nr)    << AUXADC_CHANNEL_BITS) | \
     ((value) << AUXADC_VALUE_BITS))

#define AUXADC_CHANNEL(nr)    (((nr) >> AUXADC_CHANNEL_BITS) & 0x1f)
#define AUXADC_VALUE(nr)      (((nr) >> AUXADC_VALUE_BITS) & 0xfff)
...
static long auxadc_cali_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int temp, auxadc_channel, auxadc_value;
	int data[4], ext_chan_value[6], ext_adc_pack_value[6];
	...
	auxadc_channel = AUXADC_CHANNEL(temp);
	auxadc_value = AUXADC_VALUE(temp);
	...
	case CMD_AUXADC_READ_ONE_CHAN:
		IMM_auxadc_GetOneChannelValue(auxadc_channel, data, &auxadc_value);
		if(put_user((AUXADC_PACK(auxadc_channel, auxadc_value)), (int __user *)arg))
		{
			 goto fail;
		}
		break;

	case CMD_AUXADC_READ_ALL_EXT_CHAN:
		/*external channel number:AUXIN0~5:0,1,9,12,13,14*/
		IMM_auxadc_GetOneChannelValue(0, data, &ext_chan_value[0]);
		IMM_auxadc_GetOneChannelValue(1, data, &ext_chan_value[1]);
		IMM_auxadc_GetOneChannelValue(9, data, &ext_chan_value[2]);
		IMM_auxadc_GetOneChannelValue(12, data, &ext_chan_value[3]);
		IMM_auxadc_GetOneChannelValue(13, data, &ext_chan_value[4]);
		IMM_auxadc_GetOneChannelValue(14, data, &ext_chan_value[5]);

		ext_adc_pack_value[0] = AUXADC_PACK(0, ext_chan_value[0]);
		ext_adc_pack_value[1] = AUXADC_PACK(1, ext_chan_value[1]);
		ext_adc_pack_value[2] = AUXADC_PACK(9, ext_chan_value[2]);
		ext_adc_pack_value[3] = AUXADC_PACK(12, ext_chan_value[3]);
		ext_adc_pack_value[4] = AUXADC_PACK(13, ext_chan_value[4]);
		ext_adc_pack_value[5] = AUXADC_PACK(14, ext_chan_value[5]);

		if(copy_to_user((void __user *)arg, ext_adc_pack_value, 24)) {
			goto fail;
		}
		break;
...
{
	pr_debug("auxadc_cali_open enter\n");
	return 0;
}

unsigned int auxadc_select_channel[4] = {0};

static ssize_t auxadc_cali_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	pr_info("auxadc_cali_read enter!\n");
	i = auxadc_select_channel[0];
	ret = IMM_auxadc_GetOneChannelValue(i, data, rawvalue);

	pr_info("[adc_api]: 1!imm mode raw data => channel[%d] = %d,%d\n",i,rawvalue[0],*rawvalue);
	if(copy_to_user(buf, data, count)) {
		ret = - EFAULT;
	} else {
		*ppos += count;
		ret = count;
		rawvalue[0] = rawvalue[0] * VOLTAGE_FULL_RANGE / AUXADC_PRECISE;
		pr_info("[adc_api]: 2!imm mode raw data => channel[%d] = %d,%d\n",i,rawvalue[0],*rawvalue);
	}

	return ret;
}

static ssize_t auxadc_cali_write(struct file *file, const char __user *buf, size_t size,loff_t *ppos)
{
	unsigned int count = size;
	int data[4] = { 0 };
	int ret = 0;

	pr_info("auxadc_cali_write enter!\n");
	pr_info("buf[0] = %d!\n",*buf);
	if(copy_from_user(data, buf, count)) {
		...
		*ppos += count;
		ret = count;
		pr_info("copy_from_user success!\n");
		pr_info("copy data(0x%x) to user buffer!\n",data[0]);
	}

	/*get select channel from user space*/
	auxadc_select_channel[0] = data[0];
	pr_info("auxadc_select_channel[0] = 0x%x!\n",auxadc_select_channel[0]);
...
{
	pr_debug("auxadc_cali_release enter\n");
	return 0;
}
...
static const struct file_operations auxadc_cali_fops = {
	...
	.read = auxadc_cali_read,
	.write = auxadc_cali_write,
