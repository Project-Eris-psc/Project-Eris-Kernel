...
*
***************************************************************/
static int nand_read_page_bmt(u32 page, u8 *dat, u8 *oob)
{
	return mtk_nand_exec_read_page(mtd_bmt, page, PAGE_SIZE_BMT, dat, oob);
}

static bool nand_block_bad_bmt(u64 offset)
{
	return mtk_nand_block_bad_hw(mtd_bmt, offset);
}

static bool nand_erase_bmt(u64 offset)
{
	int status;
...
static int mark_block_bad_bmt(u64 offset)
{
	return mtk_nand_block_markbad_hw(mtd_bmt, offset);	/* mark_block_bad_hw(offset); */
}

static bool nand_write_page_bmt(u32 page, u8 *dat, u8 *oob)
{
	return mtk_nand_exec_write_page(mtd_bmt, page, PAGE_SIZE_BMT, dat, oob);
}
...
*******************************************************************/
static bmt_struct *reconstruct_bmt(bmt_struct *bmt)
{
	int i;
