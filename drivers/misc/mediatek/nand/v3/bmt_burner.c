...
{
	int ret;
	ret = mtk_nand_exec_read_page(&bmtd.host->mtd, page, bmtd.pg_size, dat, fdm);
...
static inline int bbt_nand_write(unsigned int page, unsigned char *dat,
		unsigned char *fdm)
{
	return mtk_nand_exec_write_page(&bmtd.host->mtd, page, bmtd.pg_size, dat, fdm);
}

static inline int bbt_nand_erase(unsigned short block)
{
	int status;
	status = mtk_nand_erase_hw(&bmtd.host->mtd, blk_pg(block));
...
static int scan_bad_blocks(struct bbbt *bbt)
{
	...
	if (i < 1) {
		pr_err("nand: FATAL ERR: no space to store BMT!!\n");
		return -1;
	}

	pr_notice("[BBT] %d available blocks in BMT pool\n", i);
...
static struct bbbt *scan_bmt(unsigned short block)
{
	...
	if (is_valid_bmt(nand_bbt_buf, nand_spare_buf)) {
		...
		if (bmtd.bmt_blk_idx == 0) {
			pr_crit("[BBT] FATAL ERR: bmt block index is wrong!\n");
			return NULL;
		}
		pr_notice("[BBT] BMT.v2 is found at 0x%x\n", block);
...
static unsigned short upload_bmt(struct bbbt *bbt, int n)
{
	...
	if (n < 0 || bbt->bmt_tbl[n].mapped == NORMAL_MAPPED) {
		pr_crit("nand: FATAL ERR: no space to store BMT!\n");
...
error:
	pr_crit("nand: FATAL ERR: BMT pool is run out!\n");
	return 0;
}
		...
		unsigned char *dat, unsigned char *oob)
{
	...
	if (bbt_nand_erase(block)) {
		pr_warn("nand: warn: Erase block 0x%x failed\n", block);
		return -1;
	}

	if (bbt_nand_write(page, dat, oob)) {
		pr_warn("nand: warn: Write page 0x%x failed\n", page);
		return -1;
	}
...
{
	if (is_bmt_ready()) {
		BBT_LOG("0x%x mapped to 0x%x", block, bmtd.bbt->bb_tbl[block]);
		return bmtd.bbt->bb_tbl[block];
	} else {
		pr_warn("nand: Warning: the BMT table has not been initialized\n");
		...
		unsigned short pmt_block)
{
	...
	if (!nand_bbt_buf || !nand_data_buf || !nand_spare_buf) {
		pr_crit("nand: FATAL ERR: allocate buffer failed!\n");
		ret = -1;
		goto error;
	}
	...
	if (!bbt) {
		...
		if (bmtd.total_blks > BB_TABLE_MAX + BMT_TABLE_MAX) {
			pr_crit("nand: FATAL: Too many blocks, can not support!\n");
			ret = -1;
			goto error;
		}
		...
		bmtd.bmt_blk_idx = upload_bmt(bbt, bmtd.bmt_blk_idx);
		if (bmtd.bmt_blk_idx == 0)
			pr_warn("nand: Warning: no available block in BMT pool!\n");
