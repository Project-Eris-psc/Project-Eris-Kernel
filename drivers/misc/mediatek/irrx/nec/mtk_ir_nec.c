...
static int mtk_ir_nec_suspend(void *preserve)
{
	 ...
	 * 1000,0000: IRL7 & IRM7
	 */
	IR_WRITE32(IRRX_WAKECLR, 0x1); /*wake up signal clear */
	IR_WRITE32(IRRX_WAKEEN, 0x1);  /* bit0=1: Wake up by special key */
	IR_WRITE32(IRRX_IREXPEN, 0x102); /* bit8: Enable except bit count, bit[7:0] set except register */
	IR_WRITE32(IRRX_EXPBCNT, 0x21);  /* bit[5:0]: set wake up except bit count, same to 0x0 bit[5:0] */
	IR_WRITE32(IRRX_PDWNOUT, 0x1);   /* pdwn_out */
	IR_WRITE32(IRRX_ENEXP_IRM, MTK_NEC_EXP_IRM_BIT_MASK);  /* SET ENEXP_IRM */
	IR_WRITE32(IRRX_ENEXP_IRL, MTK_NEC_EXP_IRL_BIT_MASK);  /* SET ENEXP_IRL */
	IR_WRITE32(IRRX_EXP_IRM1, MTK_NEC_EXP_POWE_KEY1);  /* NEC power key */
...
{
	IR_NEC_FUN();
	IR_WRITE32(IRRX_IREXPEN, 0x0); /* bit8: Enable except bit count, bit[7:0] set except register */
	IR_WRITE32(IRRX_PDWNOUT, 0x0); /* pdwn_out */
	IR_WRITE32(IRRX_WAKECLR, 0x1); /*wake up signal clear */
