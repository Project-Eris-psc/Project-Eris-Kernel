...
static int mtk_disp_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	err = clk_enable(mdp->clk_pwm_sel);
	if (err < 0)
		return err;

	err = clk_enable(mdp->clk_pwm_mm);
	if (err < 0)
		goto disable_clk_pwm_sel;

	err = clk_enable(mdp->clk_main);
	if (err < 0)
		goto disable_clk_pwm_mm;
...
disable_clk_main:
	clk_disable(mdp->clk_main);
disable_clk_pwm_mm:
	clk_disable(mdp->clk_pwm_mm);
disable_clk_pwm_sel:
	clk_disable(mdp->clk_pwm_sel);
	return err;
}
...
static int mtk_disp_pwm_probe(struct platform_device *pdev)
{
	...
	mdp->clk_pwm_sel = devm_clk_get(&pdev->dev, "pwm_sel");
	if (IS_ERR(mdp->clk_pwm_sel))
		return PTR_ERR(mdp->clk_pwm_sel);

	mdp->clk_pwm_mm = devm_clk_get(&pdev->dev, "pwm_mm");
	if (IS_ERR(mdp->clk_pwm_mm))
		return PTR_ERR(mdp->clk_pwm_mm);
	...
	ret = clk_prepare(mdp->clk_pwm_sel);
	if (ret < 0)
		return ret;

	ret = clk_prepare(mdp->clk_pwm_mm);
	if (ret < 0)
		goto disable_clk_pwm_sel;
...
static int mtk_disp_pwm_remove(struct platform_device *pdev)
{
	...
	clk_unprepare(mdp->clk_pwm_sel);
	clk_unprepare(mdp->clk_pwm_mm);
	clk_unprepare(mdp->clk_mm);
	clk_unprepare(mdp->clk_main);
