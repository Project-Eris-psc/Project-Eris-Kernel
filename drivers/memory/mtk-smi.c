...
static int mtk_smi_enable(const struct mtk_smi *smi)
{
	...
	if (ret) {
		pr_err("%s prepare apb failed %d\n", __func__, ret);
		goto err_put_pm;
	}

	ret = clk_prepare_enable(smi->clk_smi);
	if (ret) {
		pr_err("%s prepare smi failed %d\n", __func__, ret);
...
static void mtk_smi_disable(const struct mtk_smi *smi)
{
	...
	ret = pm_runtime_put_sync(smi->dev);
	if (ret < 0)
		pr_err("%s faild %d\n", __func__, ret);
}
...
int mtk_smi_larb_get(struct device *larbdev)
{
	...
	if (ret) {
		dev_err(larb->smi_common_dev, "Failed to get the smi_common device\n");
		return ret;
	}
	...
	if (ret) {
		dev_err(larbdev, "Failed to get the smi device\n");
