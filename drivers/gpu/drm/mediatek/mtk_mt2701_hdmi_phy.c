...
int hdmi_power_pin, hdmi_over_current_pin;
int hdmi_oc_status = 0;
int hdmi_oc_times = 0;
...
static irqreturn_t hdmi_over_current_irq(int irq, void *dev_id)
{
	if(!gpio_get_value(hdmi_over_current_pin)){
		gpio_set_value(hdmi_power_pin,0);
		if (hdmi_oc_status == 0) {
			pr_err("[HDMI] 5V power overcurrent!\n");
			hdmi_oc_status = 1;
		}
	} else if(gpio_get_value(hdmi_over_current_pin)){
		gpio_set_value(hdmi_power_pin,1);
		if (hdmi_oc_status == 1) {
			pr_err("[HDMI] 5V power normal!\n");
			hdmi_oc_status = 0;
		}
	}
	if (((hdmi_oc_times %100) == 0) || ((hdmi_oc_times %100) == 1)) {
		if (hdmi_oc_status == 1)
			send_event(1);
		else
			send_event(0);
...
static int mtk_hdmi_phy_probe(struct platform_device *pdev)
{
	...
	/* hdmi power control*/
	hdmi_power_pin = of_get_named_gpio(dev->of_node, "hdmi_power_gpio", 0);
	hdmi_over_current_pin = of_get_named_gpio(dev->of_node, "hdmi_over_current_gpio", 0);
	gpio_direction_output(hdmi_power_pin, 0);
	gpio_direction_input(hdmi_over_current_pin);

	ret = request_irq(gpio_to_irq(hdmi_over_current_pin),
		hdmi_over_current_irq,
		IRQ_TYPE_EDGE_BOTH,
		"hdmi over current", &pdev->dev);
